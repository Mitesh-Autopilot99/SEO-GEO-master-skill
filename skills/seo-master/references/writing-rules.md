...
Before applying any rule below, check whether a brand brief or voice/style guide exists for this site (a brand guidelines doc, a tone-of-voice spec, a previous on-brand sample, or anything the user has supplied). If one exists, read it and treat it as the canonical voice spec.
...
- BAD: "This tool isn't a cheaper analytics suite — it's a different category."
- GOOD: "This tool isn't a cheaper analytics suite. It's a different category."
...
- [ ] **Zero em-dashes (`—`) in the body.** Run `grep -c "—" article.md`, which must return 0. Same for en-dashes used as punctuation (` – `) and double-hyphens (`--`).
